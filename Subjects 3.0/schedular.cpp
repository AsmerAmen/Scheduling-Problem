 ...
 * @return: void
 */
void Schedular::generateRegister(void){
	srand( time(0) );
	int counter;
	int x;
 ...
 * @return: void
 */
void Schedular::obtainConflict(void){
...
void Schedular::findNonConflictMatrix(void){
...
void Schedular::printStudentReg(void){
	for(int i=0; i<NUM_STUDENTS; i++){
		for(int j=0; j<NUM_SUBJECTS; j++)
			cout<<stdReg[i][j];
		cout<<endl<<endl;
...
void Schedular::printConflictMatrix(void){		
	for(int sub=0; sub<NUM_SUBJECTS; sub++){
		for(int subj=0; subj<NUM_SUBJECTS; subj++)
...
void Schedular::printNonConflictMatrix(void){
	for(int i=0; i<NUM_SUBJECTS; i++){
		cout<<"Sub["<<i<<"]: ";
		for(int j=0; j<NUM_SUBJECTS; j++){
...
void Schedular::addEdge(int v, int w)
{
    adj[v].push_back(w);
